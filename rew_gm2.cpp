...
void read_twopt_rew(FILE* stream, double*** to_write, generic_header head) {
...
int main(int argc, char** argv) {
    error(argc != 9, 1, "nissa_mpcac ",
        "usage:./exe -p path file -bin $bin  jack/boot  mu\n "
    ...
    //////////////////////////////////// setup output files
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_output", option[3], option[6], argv[8]);
    printf("writing output in :\n %s \n", namefile);
    FILE* outfile = open_file(namefile, "w+");

    mysprintf(namefile, NAMESIZE, "%s/jackknife/%s_%s_%s", option[3], option[4],
        option[6], argv[8]);
    ...
    head.ncorr = 4;
    //////////////////////////////////// confs
    int nmax_corr = head.ncorr * 2 + 1;
    double**** data = calloc_corr(confs, nmax_corr, head.T);
    double**** data_1 = calloc_corr(218, nmax_corr, head.T);
    double**** data_bin_1;
    double**** conf_jack_1;
    ...
    for (int iconf = 0; iconf < head_rew.Njack; iconf++) {
        read_twopt_rew(infile_rew, data_rew[iconf], head_rew);
    }
    ...
    error(confs != head_rew.Njack, 1, "main", "head.Njack %d != head_rew.Njack %d", head.Njack, head_rew.Njack);
    //////////////////////////////////////////////////////////////
    // reweightin
    //////////////////////////////////////////////////////////////
    ...
    double**** data_bin = bin_intoN(data, nmax_corr, head.T, confs, bin);//binning(confs, head.ncorr, head.T, data, bin);
    ...
    std::vector<double*> muiso(3);
    line_read_param(option, "muliso", mean, err, seed, namefile_plateaux);
    muiso[0] = myres->create_fake(mean, err, seed);


    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    // print all the effective masses correlators
    // set the option to not read for a plateaux
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_meff_correlators", option[3],
        option[6], argv[8]);
    FILE* outfile_meff_corr = open_file(namefile, "w+");
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_raw_correlators", option[3],
        option[6], argv[8]);
    FILE* outfile_raw_corr = open_file(namefile, "w+");
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_shifted_correlators", option[3],
        option[6], argv[8]);
    FILE* outfile_shifted_corr = open_file(namefile, "w+");
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_log_meff_shifted", option[3],
        option[6], argv[8]);
    FILE* outfile_log_meff_shifted = open_file(namefile, "w+");
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_gamma", option[3],
        option[6], argv[8]);
    FILE* out_gamma = open_file(namefile, "w+");

    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_HLT_kernel", option[3],
        option[6], argv[8]);
    FILE* outfile_HLT_kernel = open_file(namefile, "w+");
    mysprintf(namefile, NAMESIZE, "%s/out/%s_%s_HLT_AoverB", option[3],
        option[6], argv[8]);
    ...
    //////////////////////////////////////////////////////////////
    // bounding meff_t  reweighting
    //////////////////////////////////////////////////////////////
    ibound = 3;
    int id_VV_TM_rew = head.ncorr + 0; // the index of the reweighted correlator
    int id_VV_OS_rew = head.ncorr + 1; // the index of the reweighted correlator
    double* amu_TM_bound_meff_t_rew = compute_amu_bounding(conf_jack, id_VV_TM_rew, Njack, ZA, a_fm, q2ud, int_scheme, outfile, "amu_{bound_meff_t}_(TM)_rew",
        resampling, isub, ibound, Mpi, nullptr, head.L);
    write_jack(amu_TM_bound_meff_t_rew, Njack, jack_file); check_correlatro_counter(10);
    printf("amu_TM(bound_meff_t)_rew = %g  %g \n", amu_TM_bound_meff_t_rew[Njack - 1], myres->comp_error(amu_TM_bound_meff_t_rew));

    double* amu_OS_bound_meff_t_res = compute_amu_bounding(conf_jack, id_VV_OS_rew, Njack, ZV, a_fm, q2ud, int_scheme, outfile, "amu_{bound_meff_t}_(OS)_rew",
        resampling, isub, ibound, Mpi, nullptr, head.L);
    write_jack(amu_OS_bound_meff_t_res, Njack, jack_file); check_correlatro_counter(11);
    printf("amu_OS(bound_meff_t)_rew = %g  %g \n", amu_OS_bound_meff_t_res[Njack - 1], myres->comp_error(amu_OS_bound_meff_t_res));



    double* amu_TM_rew = compute_amu_full(conf_jack, id_VV_TM_rew, Njack, ZA, a_fm, q2ud, int_scheme, outfile, "amu_{full}_(TM)_rew", resampling, isub);

    double* amu_OS_rew = compute_amu_full(conf_jack, id_VV_OS_rew, Njack, ZV, a_fm, q2ud, int_scheme, outfile, "amu_{full}_(OS)_rew", resampling, isub);

    //////////////////////////////////////////////////////////////
    // derivative
    //////////////////////////////////////////////////////////////
    double *der = myres->create_copy(amu_TM_bound_meff_t);
    myres->sub(der, der, amu_TM_bound_meff_t_rew);
    double dmu = head_rew.mus[0] - head_rew.oranges[0];
    myres->div(der, der, dmu);
    write_jack(der, Njack, jack_file);
    printf("derivative TM= %g  %g \n", der[Njack - 1], myres->comp_error(der));
    print_result_in_file(outfile, der, "damu_TM/dmus", 0, 0, 0);

    myres->mult(der, der, muiso[0]);
    write_jack(der, Njack, jack_file);
    printf("mul*damu_TM/dmu= %g  %g \n", der[Njack - 1], myres->comp_error(der));
    print_result_in_file(outfile, der, "damu_TM/dmus", 0, 0, 0);


    myres->copy(der,amu_OS_bound_meff_t);
    myres->sub(der, der, amu_OS_bound_meff_t_res);
    myres->div(der, der, dmu);
    write_jack(der, Njack, jack_file);
    printf("derivative OS= %g  %g \n", der[Njack - 1], myres->comp_error(der));
    print_result_in_file(outfile, der, "damu_OS/dmus", 0, 0, 0);

    myres->mult(der, der, muiso[0]);
    write_jack(der, Njack, jack_file);
    printf("mul*damu_OS/dmu= %g  %g \n", der[Njack - 1], myres->comp_error(der));
    print_result_in_file(outfile, der, "damu_OS/dmus", 0, 0, 0);
    ...
    fit_info_silent.codeplateaux = true;
    fit_info_silent.tmin = 1;
    fit_info_silent.tmax = 1;
    tmp = plateau_correlator_function(
        option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
        namefile_plateaux, outfile, 0, "VKVK_TM", identity, jack_file, fit_info_silent);
    free(tmp);
    tmp = plateau_correlator_function(
        option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
        namefile_plateaux, outfile, 1, "VKVK_OS", identity, jack_file, fit_info_silent);
    free(tmp);

    tmp = plateau_correlator_function(
        option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
        namefile_plateaux, outfile, id_VV_TM_rew, "VKVK_TM_rew", identity, jack_file, fit_info_silent);
    free(tmp);
    tmp = plateau_correlator_function(
        option, kinematic_2pt, (char*)"P5P5", conf_jack, Njack,
        namefile_plateaux, outfile, id_VV_OS_rew, "VKVK_OS_rew", identity, jack_file, fit_info_silent);
    free(tmp);
